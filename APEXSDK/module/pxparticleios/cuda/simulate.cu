...
BOUND_S2_KERNEL_BEG(simulateKernel,
	...
	((float*, g_benefit))((float4*, g_pxPosition))((float4*, g_pxVelocity))((float4*, g_pxCollisionNormals))((float*, g_pxDensity))((unsigned int*, g_pxFlags))
	((GridDensityParams, simParams))
)
...
BOUND_S2_KERNEL_BEG(simulateApplyFieldKernel,
	...
	((APEX_MEM_BLOCK(float4), g_pxCollisionNormals))((APEX_MEM_BLOCK(float), g_pxDensity))((APEX_MEM_BLOCK(unsigned int), g_pxFlags))
	((GridDensityParams, simParams))
)
...
BOUND_S2_KERNEL_BEG(testKernel,
	...
	{
		int mult;
		multHandle.fetch(KERNEL_CONST_STORAGE, mult);
