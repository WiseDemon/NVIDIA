...
template <uint VectorSize, bool FullBlock>
							  ...
							  uint remainWarps)
{
	...
	const unsigned int WarpStride = WarpsPerBlock + NEW_SORT_WARP_STRIDE_PAD;
							  ...
							  uint remainDataCount, uint blockPos)
{
	...
	localSortStep<VectorSize, FullBlock>(count, startBit, key, value, sKeys, sValues, sWarpCounters, sdata, remainWarps);
...
template <uint VectorSize, bool doFlip>
inline __device__ void radixSortStep1(uint count, uint startBit, uint* inpKeys, uint* inpValues, uint* outKeys, uint* outValues, uint* tempScan, volatile uint* sdata)
{
	const unsigned int Log2ElemsPerThread = VectorTT<VectorSize>::Log2Size;
	const unsigned int ElemsPerThread = VectorTT<VectorSize>::Size;
	...
	volatile uint* sWarpCounters = sdata + BlockSize*2; /* size = [KEY_DIGITS_PER_STEP * WarpStride] */
	...
	for (int remainWarps = DataWarpsCount; remainWarps > 0; remainWarps -= WarpsPerBlock, blockPos += BlockSize)
	{
		...
		if (warpIdx < remainWarps)
		{
			...
			{
				processReduce(accum, sdata, sWarpCounters);
...
template <uint VectorSize, bool doFlip>
inline __device__ void radixSortStep2(int count, int blockCount, uint* tempScan, volatile uint* sdata)
{
	//__shared__ volatile uint sdata[BlockSize*2];
...
template <uint VectorSize, bool doFlip>
inline __device__ void radixSortStep3(uint count, uint startBit, uint* inpKeys, uint* inpValues, uint* outKeys, uint* outValues, uint* tempScan, volatile uint* sdata)
{
	const unsigned int Log2ElemsPerThread = VectorTT<VectorSize>::Log2Size;
	typedef typename VectorTT<VectorSize>::AccessType ElemAccessType;
	...
	//temp shared memory for scan
	//__shared__ volatile uint sdata[BlockSize*2];
	uint* sKeys = (uint*)sdata + BlockSize*2; /* size = [BlockDataSize] */
	uint* sValues = sKeys + BlockDataSize; /* size = [BlockDataSize] */
	uint* sWarpCounters = sValues + BlockDataSize; /* size = [KEY_DIGITS_PER_STEP * WarpStride + 1] (+1 here is to store total scan at the end) */
	...
	{
		localSortStepBlock<VectorSize, doFlip, true>(
	...
	{
		localSortStepBlock<VectorSize, doFlip, false>(
			count, startBit, inpKeys, inpValues, outKeys, outValues, 
			sKeys, sValues, sWarpCounters, sdata, sCounters,
...
template <uint VectorSize, bool doFlip>
inline __device__ void radixSortBlock(uint count, uint startBit0, uint startBit1, uint* inpKeys, uint* inpValues, volatile uint* sdata)
{
	const unsigned int Log2ElemsPerThread = VectorTT<VectorSize>::Log2Size;
	const unsigned int ElemsPerThread = VectorTT<VectorSize>::Size;
	typedef typename VectorTT<VectorSize>::AccessType ElemAccessType;
	...
	{
		localSortStep<VectorSize, false>(count, startBit, key, value, sKeys, sValues, sWarpCounters, sdata, DataWarpsCount);
...
BOUND_KERNEL_BEG(newRadixSortBlockKernel,
	uint count, uint bitCount, uint startBit, uint* inpKeys, uint* inpValues
)
	extern __shared__ volatile uint sdata[];

	radixSortBlock<NEW_SORT_VECTOR_SIZE, false>(count, startBit, startBit + bitCount, inpKeys, inpValues, sdata);

BOUND_KERNEL_END()

BOUND_KERNEL_BEG(newRadixSortStepKernel,
	...
	if (phase == 1)
	{
		radixSortStep1<NEW_SORT_VECTOR_SIZE, false>(count, startBit, inpKeys, inpValues, outKeys, outValues, tempScan, sdata);
	}
	else if (phase == 2)
	{
		radixSortStep2<NEW_SORT_VECTOR_SIZE, false>(count, gridSize, tempScan, sdata);
	}
	else
	{
		radixSortStep3<NEW_SORT_VECTOR_SIZE, false>(count, startBit, inpKeys, inpValues, outKeys, outValues, tempScan, sdata);
	}
BOUND_KERNEL_END()
