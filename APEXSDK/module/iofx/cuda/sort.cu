...
template <uint nbits, bool doFlip>
									  ...
									  volatile uint* sKeys, volatile uint* sValues, const uint blockBeg, const uint blockEnd)
{
	...
	for (uint blockPos = blockBeg; blockPos < blockEnd; blockPos += BlockSize)
	{
		...
		__syncthreads();
		radixSortBlock<nbits> (sKeys, sValues, key, value, startbit);
...
#define SORT_STEP_KERNEL_SETUP(count) \
	...
	extern __shared__ volatile uint sdata[]; /* size = [BlockSize * 2] */ \
...
SYNC_KERNEL_BEG(radixSortSyncKernel, uint numElements,
	...
	for (uint startbit = startbit0; startbit < startbit0 + keyBits; startbit += RADIX_SORT_NBITS) {
		radixSortStep<RADIX_SORT_NBITS, false> (keys, values, tempKeys, tempValues, numElements, startbit, g_temp);
	}
SYNC_KERNEL_END()


BOUND_KERNEL_BEG(radixSortStepKernel,
	...
	if (phase == 1)
	{
		radixSortStep1<RADIX_SORT_NBITS, false>(
	...
	{
		radixSortStep3<RADIX_SORT_NBITS, false>(
			keys, values, tempKeys, tempValues, _threadCount, startBit, g_temp,
			sKeys, sValues, blockBeg, blockEnd);
	}
BOUND_KERNEL_END()
