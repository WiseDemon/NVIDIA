	...
	volatile unsigned int* sWarpFirstActorID, volatile float* sWarpFirstBounds)
{
	...
	const unsigned int BlockPitch = (BlockSize + BBOX_BLOCK_BOUNDS_PAD);
	const unsigned int WarpPitch = (WARP_SIZE + BBOX_WARP_BOUNDS_PAD);
	...
	if (warpBeg < warpEnd)
	{
		...
		{
			//read data
			unsigned int actorID = g_actorID[pos];
		...
		if (lastWarpSize > 0)
		{
			unsigned int actorID = (pos < warpEnd) ? g_actorID[pos] : UINT_MAX;
			sBlockActorID[idx] = actorID;
...
#define BBOX_KERNEL_SETUP() \
	...
	volatile unsigned int* sBlockActorID = sdata; /* size = [BlockSize] */ \
	volatile float*        sBlockBounds = (volatile float*)(sdata + BlockSize); /* size = [(BlockSize + BBOX_BLOCK_BOUNDS_PAD) * 6] */ \
	const unsigned int WarpPitch = (WARP_SIZE + BBOX_WARP_BOUNDS_PAD); \
...
SYNC_KERNEL_BEG(bboxSyncKernel,
	unsigned int count,
	unsigned int* g_actorID,
	unsigned int* stateToInput,
	const float4* g_positionMass,
