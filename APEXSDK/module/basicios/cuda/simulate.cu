...
#define APEX_CUDA_STORAGE_NAME simulateStorage
#include "include/common.h"
#include "common.cuh"

using namespace physx::apex;
using namespace physx::apex::basicios;
#include "../include/BasicIosCommon.h"
#include "include/simulate.h"
#include "../include/BasicIosCommonSrc.h"
...
__device__ void simulate(unsigned int targetCount,
	...
	float* g_benefit, InplaceHandle<SimulationParams> simParamsHandle, FieldAccessor& fieldAccessor)
{
	const unsigned int BlockSize = blockDim.x;
	const unsigned int WarpsPerBlock = (BlockSize >> LOG2_WARP_SIZE);

	extern __shared__ histogram_t s_Hist[]; /* size = [HISTOGRAM_SIMULATE_BIN_COUNT * WarpsPerBlock] */
	...
	simParamsHandle.fetch(KERNEL_CONST_STORAGE, simParams);

	InjectorParamsArray injectorParamsArray;
	injectorParamsArrayHandle.fetch(KERNEL_CONST_STORAGE, injectorParamsArray);
	...
	for (unsigned int dstIdx = BlockSize*blockIdx.x + threadIdx.x; dstIdx < targetCount; dstIdx += BlockSize*gridDim.x)
	{
		...
		unsigned int outInj;
		float benefit = simulateParticle INPLACE_TEMPL_ARGS_VAL (
			&simParams, KERNEL_CONST_STORAGE, injectorParamsArray,
			deltaTime, gravity, eyePos,
			isNewParticle, srcIdx, dstIdx,
			g_positionMass, g_velocityLife, g_iofxActorIDs,
			g_lifeSpan, g_lifeTime, g_injector, g_collisionNormalFlags, g_userData,
			fieldAccessor, outInj
		);
...
BOUND_S2_KERNEL_BEG(simulateKernel,
	((unsigned int, lastCount))((float, deltaTime))((physx::PxVec3, gravity))((physx::PxVec3, eyePos))
	((InplaceHandle<InjectorParamsArray>, injectorParamsArrayHandle))((unsigned int, injectorCount))
	((unsigned int*, g_holeScanSum))((unsigned int*, g_moveCount))((unsigned int*, g_tmpHistogram))((unsigned int*, g_InjectorsCounters))
	((float4*, g_positionMass))((float4*, g_velocityLife))((float4*, g_collisionNormalFlags))((unsigned int*, g_userData))
	((float*, g_lifeSpan))((float*, g_lifeTime))((unsigned int*, g_injector))((NiIofxActorID*, g_iofxActorIDs))
	((float*, g_benefit))((InplaceHandle<SimulationParams>, simParamsHandle))
)
	class FieldAccessor {
	public:
	...
	simulate INPLACE_TEMPL_ARGS_VAL (_threadCount,
		lastCount, deltaTime, gravity, eyePos,
		injectorParamsArrayHandle, injectorCount, 
		g_holeScanSum, g_moveCount, g_tmpHistogram, g_InjectorsCounters, 
		g_positionMass, g_velocityLife, g_collisionNormalFlags, g_userData,
		g_lifeSpan, g_lifeTime, g_injector, g_iofxActorIDs,
		g_benefit, simParamsHandle, fieldAccessor
	);
BOUND_S2_KERNEL_END()

BOUND_S2_KERNEL_BEG(simulateApplyFieldKernel,
	((unsigned int, lastCount))((float, deltaTime))((physx::PxVec3, gravity))((physx::PxVec3, eyePos))
	((InplaceHandle<InjectorParamsArray>, injectorParamsArrayHandle))((unsigned int, injectorCount))
	...
	((APEX_MEM_BLOCK(float), g_benefit))((InplaceHandle<SimulationParams>, simParamsHandle))
)
	class FieldAccessor {
	public:
		__device__ PX_INLINE void operator() (unsigned int srcIdx, physx::PxVec3& velocityDelta ) {
...
BOUND_KERNEL_BEG(mergeHistogramKernel,
...
BOUND_KERNEL_BEG(stateKernel,
	unsigned int lastCount, unsigned int targetCount,
	unsigned int* g_moveCount,
	unsigned int* g_inStateToInput, unsigned int* g_outStateToInput
)
	__shared__ unsigned int moveCount;
	if (threadIdx.x == 0) {
		moveCount = g_moveCount[0];
...
BOUND_KERNEL_BEG(gridDensityGridClearKernel,
...
BOUND_KERNEL_BEG(gridDensityGridFillKernel,
	float4* positionMass,
	float* gridDensityGrid,
	GridDensityParams simParams
	)
...
BOUND_KERNEL_BEG(gridDensityGridApplyKernel,
	float* density,
	float4* positionMass,
	float* gridDensityGrid,
	GridDensityParams simParams
...
BOUND_KERNEL_BEG(gridDensityGridFillFrustumKernel,
	float4* positionMass,
	float* gridDensityGrid,
	GridDensityParams simParams,
	::physx::PxMat44 mat,
...
BOUND_KERNEL_BEG(gridDensityGridApplyFrustumKernel,
	float* density,
	float4* positionMass,
	float* gridDensityGrid,
	GridDensityParams simParams,
...
// ************* LowPass*******************
FREE_KERNEL_BEG(gridDensityGridLowPassKernel,
