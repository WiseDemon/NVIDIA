...
namespace physx
{
	namespace apex
	{
		namespace basicios
		{
			...
			__device__
#endif
			PX_INLINE physx::PxU32 handleCollisions(const SimulationParams* params, INPLACE_STORAGE_ARGS_DEF, physx::PxVec3& position, physx::PxVec3& velocity, physx::PxVec3& normal)
			{
				...
				PxU32 numTriMeshes = params->trimeshes.getSize();
				for (PxU32 i = 0; i < numTriMeshes; ++i)
				{
				...
				PxU32 numConvexMeshes = params->convexMeshes.getSize();
				for (PxU32 i = 0; i < numConvexMeshes; ++i)
				{
				...
				PxU32 numBoxes = params->boxes.getSize();
				for (PxU32 i = 0; i < numBoxes; ++i)
				{
				...
				PxU32 numCapsules = params->capsules.getSize();
				for (PxU32 i = 0; i < numCapsules; ++i)
				{
					...
					const physx::PxF32 closestX = physx::PxClamp(localPosition.x, -data.halfHeight, data.halfHeight);
					physx::PxVec3 localNormal(localPosition.x - closestX, localPosition.y, localPosition.z);
				...
				PxU32 numSpheres = params->spheres.getSize();
				...
				PxU32 numHalfSpaces = params->halfSpaces.getSize(); 
				for (PxU32 i = 0; i < numHalfSpaces; ++i)
				{
