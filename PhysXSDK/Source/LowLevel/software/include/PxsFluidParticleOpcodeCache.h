...
#ifndef PXS_FLUID_PARTICLE_OPCODE_CACHE_H
#define PXS_FLUID_PARTICLE_OPCODE_CACHE_H
...
There are four different data layouts to optimize access speed for fewer indices and maximize the amount of triangles that can be cached.
1. regular PxBounds3 with up to 1 x 16 bit triangle indices.
2. compressed volume with up to 6 x 16 bit triangle indices.
3. compressed volume with up to 9 x 10 bit triangle indices. (The indices are compressed if the range of indices allows).
4. compressed volume with up to 3 x 32 bit triangle indices. (For large meshes).
*/
struct PxsFluidParticleOpcodeCache
{
	static const PxU32 sMaxCachedTriangles = 9;
	static const PxU32 sMaxCachedConvexFaces = 11;
	...
	PX_FORCE_INLINE	PxsFluidParticleOpcodeCache&	operator=(const PxsFluidParticleOpcodeCache& p)	
	{
		const PxU32* src = (PxU32*)&p;
		PxU32* dist = (PxU32*)this;
		dist[0] = src[0];
		dist[1] = src[1];
		dist[2] = src[2];
		dist[3] = src[3];
	...
	{
		mTriangleCount = 0;
		mGeom = (Gu::GeometryUnion*)triangles;
	}	
	...
	PX_FORCE_INLINE void write(
		PxU16& internalParticleFlags,
		const PxBounds3& bounds,
		const QuantizationParams& quantizationParams,
		const Gu::GeometryUnion& mesh,
		const bool isSmallMesh)
	{	
		PxU32* triangles = (PxU32*)mGeom;
		...
		else
		{
			...
			else if (isSmallMesh && mTriangleCount <= 9)
			{
				...
				if (!success)
				{
					internalParticleFlags &= ~PxU16(PxvInternalParticleFlag::eGEOM_CACHE_MASK);
					return;
				}
			}
		...
		internalParticleFlags |= (PxvInternalParticleFlag::eGEOM_CACHE_BIT_0 | PxvInternalParticleFlag::eGEOM_CACHE_BIT_1);
		mGeom = &mesh;
	}

	PX_FORCE_INLINE bool read(
		PxU16& internalParticleFlags,
		PxU32& numTriangles, 
		PxU32* triangleBuffer, 
		const PxBounds3& bounds,
		const QuantizationParams& quantizationParams, 
		const Gu::GeometryUnion* mesh,
		const bool isSmallMesh) const
	{
		//cache bits:
		// (00) -> no read (invalid)
		// (01) -> read
		// (11) -> no read (can't be the case with mGeom == mesh)
		PX_ASSERT(mGeom != mesh || !(
			(internalParticleFlags & PxvInternalParticleFlag::eGEOM_CACHE_BIT_0) != 0 &&
			(internalParticleFlags & PxvInternalParticleFlag::eGEOM_CACHE_BIT_1) != 0));

		if ((internalParticleFlags & PxvInternalParticleFlag::eGEOM_CACHE_BIT_0) != 0 && mGeom == mesh)
		{
...
private:
	...
	static PX_FORCE_INLINE void dequantizeBounds(PxBounds3& bounds,
		const PxVec3& center, const PxU8 extentX, const PxU8 extentY, const PxU8 extentZ, 
		const QuantizationParams& quantizationParams)
	{
		PxVec3 extents(extentX*quantizationParams.dequantizationMultiplier, extentY*quantizationParams.dequantizationMultiplier, extentZ*quantizationParams.dequantizationMultiplier);
	...
	static PX_FORCE_INLINE void readTriangles_BaseU16_9xU10(PxU32* triangleBuffer, const PxU8* data, const PxU32 numTriangles)
	{
		PX_ASSERT(numTriangles > 6 && numTriangles <= 9);
...
PX_COMPILE_TIME_ASSERT(sizeof(PxTriangleMeshGeometryLL*) > 4 || sizeof(PxsFluidParticleOpcodeCache) == 32);

}

#endif // PXS_FLUID_PARTICLE_OPCODE_CACHE_H
