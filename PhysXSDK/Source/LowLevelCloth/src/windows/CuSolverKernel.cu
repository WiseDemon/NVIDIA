...
template <typename T> struct SharedReference;
...
public:
	...
	typedef SharedReference<T> Reference;
...
struct SharedReference
{
	...
	{ 
		float4 result;
		result.x = *mPointers[0];
		result.y = *mPointers[1];
		result.z = *mPointers[2];
		result.w = *mPointers[3];
		return result;
	}

	__device__ SharedReference& operator=(const float4& vec)
	{
		*mPointers[0] = vec.x;
		*mPointers[1] = vec.y;
		*mPointers[2] = vec.z;
	...
	typename SharedPointer<T>::Type mPointers[4];
};
...
struct GlobalPointer
{
	...
	typedef GlobalPointer<T> Reference;
	...
	// also act as float4 reference
	__device__ operator const float4&() const { return *reinterpret_cast<const float4* const&>(mPtr); }
	__device__ GlobalPointer operator=(const float4& vec) { *reinterpret_cast<float4* const&>(mPtr) = vec; return *this; }
...
struct ParticleData
{
	...
	typedef typename PointerT<float>::Reference ReferenceType;
	typedef typename PointerT<const float>::Reference ConstReferenceType;
	...
	__device__ ReferenceType operator()(uint32_t index);
	__device__ ConstReferenceType operator()(uint32_t index) const;
...
template <>
SharedReference<float> ParticleData<SharedPointer>::operator()(uint32_t index)
{
	SharedReference<float> result;
	result.mPointers[0] = mPointers[0] + index;
	result.mPointers[1] = mPointers[1] + index;
	result.mPointers[2] = mPointers[2] + index;
	result.mPointers[3] = mPointers[3] + index;
	return result;
}

template <>
SharedReference<const float> ParticleData<SharedPointer>::operator()(uint32_t index) const
{
	SharedReference<const float> result;
	result.mPointers[0] = mPointers[0] + index;
	result.mPointers[1] = mPointers[1] + index;
	result.mPointers[2] = mPointers[2] + index;
	result.mPointers[3] = mPointers[3] + index;
...
template <>
GlobalPointer<float> ParticleData<GlobalPointer>::operator()(uint32_t index)
{
	return *mPointers + index;
}

template <>
GlobalPointer<const float> ParticleData<GlobalPointer>::operator()(uint32_t index) const
{
	return *mPointers + index;
}
...
namespace
{
	...
	__device__ void integrateParticles(CurrentT& current, PreviousT& previous)
	{
		...
		for(uint32_t i = threadIdx.x; i < gClothData.mNumParticles; i += blockDim.x)
		{
			...
			if(next.w > 0.0f)
			{
				if(IsTurning)
				{
					next.x = next.x + trafo[3] +
						cur.x * trafo[15] + prev.x * trafo[ 6] + 
						cur.y * trafo[16] + prev.y * trafo[ 7] + 
						cur.z * trafo[17] + prev.z * trafo[ 8];
																								
					next.y = next.y + trafo[4] +
						cur.x * trafo[18] + prev.x * trafo[ 9] +
						cur.y * trafo[19] + prev.y * trafo[10] +
						cur.z * trafo[20] + prev.z * trafo[11];

					next.z = next.z + trafo[5] +
						cur.x * trafo[21] + prev.x * trafo[12] +
						cur.y * trafo[22] + prev.y * trafo[13] +
						cur.z * trafo[23] + prev.z * trafo[14];
				} else {
	...
	__device__ void constrainTether(CurrentT& current)
	{
		...
		float stiffness = numParticles * 
			__fdividef(gFrameData.mTetherConstraintStiffness, numTethers);
	...
	__device__ void solveFabric(CurrentT& current)
	{
		...
		for(; cIt != cEnd; ++cIt)
		{
			...
			uint32_t numConstraints = cIt->mNumConstraints;
...
#if __CUDA_ARCH__ >= 200 && !defined(_WIN64) && !defined(__x86_64__)
